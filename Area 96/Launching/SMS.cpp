...
void Student_management_service::Import_score_from_file(string lecturer_name) 
{
    int type, semester;
    ...
    if (_course.lecturer_name != lecturer_name) {
        cout << "You are not the lecturer of this course\n";
        return;
    }
    cout << "Enter file name (name.csv)\n";
    cin >> file_name;
...
void Student_management_service::Assign_one_student() 
{
    ...
    if (flag == 0)
        fnew << course_code << ',' << year << ',' << semester << ',' << student_id << '\n';
    fb.close();
    fbnew.close();
...
void Student_management_service::Assign_one_class()
{
    ...
    if (a.Is_empty())
        ...
        if (flag == 0)
            fnew << course_code << ',' << year << ',' << semester << ',' << tmp << '\n';
        fb.close();
        fbnew.close();
        system("mv course_student_new.txt course_student.txt");
    }
...
void Student_management_service::Edit_score(string lecturer_name)
{
    ...
    if (Tmp.lecturer_name != lecturer_name) {
        cout << "You are not the lecturer of this course\n";
        return;
    }
