...
void User_interface::Menu(string role) {
    ...
    if (role == "student") {
        ...
        for (int i=0;i<=2;++i)
            cout << setw(10) << i << setw(_width) << s[i] << '\n';
        for (int i=13;i<=14;++i)
            cout << setw(10) << i-10 << setw(_width) << s[i] << '\n';
        while ((cout << " Enter option\n") && (cin >> flag))
        switch(flag)
        {
            case 0:
            ...
            case 2:
                ...
                break;
            case 3:
                Service.Import_score_from_file(account_name);
                cout << endl;
                break;
            case 4:
                Service.Edit_score(account_name);
                cout << endl;
                break;
            default:
                cout << "Invalid number\n";
...
void User_interface::Change_password() {
    ...
    password = getpass("Enter new password (no space)\n");
    filebuf fb, fbnew;
